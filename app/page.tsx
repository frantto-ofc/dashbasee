...
const STORAGE_KEY = "estudio-fluxo:data:v1";
...
export default function Home() {
  const [store, setStore] = useState<AppStore>(() =>
    syncMissingProjectIncome(createSampleStore()),
  );
    ...
    try {
      ...
      if (saved) {
        ...
        if (isValidStore(parsed)) {
          // Hydrate the UI from the browser-local database after the client mounts.
          const syncedStore = syncMissingProjectIncome(parsed);
          // eslint-disable-next-line react-hooks/set-state-in-effect
          setStore(syncedStore);
          setSavedAt(syncedStore.lastUpdated);
        }
      }
    } catch {
      setToast("O backup local não pôde ser lido. Os dados de exemplo foram mantidos.");
    } finally {
      setHydrated(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Reflect the timestamp of the successful localStorage write in the UI.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileMenuOpen ? "mobile-open" : ""}`}>
        ...
        <div className="profile">
          <span className="profile-avatar">EF</span>
          <div>
            <strong>Meu estúdio</strong>
            ...
            <>
              <section className="page-heading">
                <div>
                  ...
                  <p>Seus dados ficam sob seu controle, neste navegador.</p>
                </div>
              </section>
              ...
              <section className="backup-hero">
                ...
                <div>
                  ...
                  <p>
                    O app grava cada alteração localmente. Para abrir os mesmos
                    dados em outro navegador, exporte o arquivo e importe no
                    novo ambiente.
                  </p>
                </div>
              ...
              <section className="backup-grid">
                ...
                <article className="panel backup-card">
                  ...
                  <h2>Privacidade do MVP</h2>
                  <p>
                    Nenhuma tarefa, contato, projeto ou valor financeiro é
                    enviado a um servidor nesta versão. O arquivo de backup
                    também não sai do seu dispositivo por conta própria.
                  </p>
                </article>
                <article className="panel backup-card">
              ...
              <section className="panel architecture-note">
                <div>
                  ...
                  <div>
                    <h2>Próxima evolução recomendada</h2>
                    <p>
                      Quando você quiser sincronização automática entre
                      dispositivos, adicionaremos login e banco na nuvem com
                      isolamento por usuário. O backup local continuará como
                      camada extra de segurança.
                    </p>
                  </div>
                </div>
                <span className="phase-pill">Fase 2</span>
              </section>
            </>
          )}
        </div>
      ...
      {offlineReady && <span className="sr-only">Modo offline preparado.</span>}
    </div>
  );
}
